...
def add_to_score_from_log(dirname=None, log_file=None, score_file=None, out_file=None, add_nstruc_label=False):
    """Given a dir name the function loads a score file and a log file, takes the info from thelog file and appends it to the score file
    ...
    if add_nstruc_label:
        d['description']=d['description']+"_0001"
    interface = pd.DataFrame.from_records([d], index='description')
    score = pd.read_csv(score_file, low_memory=False, header=0, skiprows=1, sep = r'\s+', skipinitialspace=True, warn_bad_lines=True)
    score.drop(columns=['SCORE:'],inplace=True)

    ##take the last row and set index on description
    score = score.iloc[[-1]].set_index('description')
    joined = score.join(interface, on="description")
    print("Saving to: "+out_file)
    joined.to_csv(out_file, sep=' ')
    return out_file
...
if __name__ == "__main__":
    ...
    add_to_score_from_dir(args.dir)